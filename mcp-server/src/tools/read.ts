...
export function registerReadTools(
  ...
  server.registerTool(
    "list_today_tasks",
    {
      ...
      description:
        "List tasks visible today (startDate <= today or no startDate), grouped by time block (morning/afternoon/evening/unscheduled). Shows task title, status, priority, project, and estimates.",
      inputSchema: {},
      annotations: {
        readOnlyHint: true,
  ...
  server.registerTool(
    "list_all_tasks",
    {
      ...
      description:
        "List all tasks with optional filters. Returns task title, status, priority, project, dates, and estimates. Only shows top-level tasks (no subtasks).",
      inputSchema: {
        status: z
          .enum(TASK_STATUSES)
          .optional()
    ...
    async ({ status, priority, projectId }) => {
      ...
      if (projectId) filters.push(`project=${projectId.slice(0, 8)}`);
  ...
  server.registerTool(
    "get_task",
    {
      ...
      description:
        "Get full details of a single task by ID, including project, labels, and subtasks.",
      inputSchema: {
    ...
    async ({ taskId }) => {
      ...
      if (task.subtasks?.length) {
        ...
        lines.push("");
        lines.push(
          `## Subtasks (${done}/${task.subtasks.length} done)`,
        );
        for (const sub of task.subtasks) {
          const check = sub.status === "done" ? "[x]" : "[ ]";
          lines.push(
            `- ${check} ${sub.title} (${sub.id.slice(0, 8)})`,
          );
        }
      }
