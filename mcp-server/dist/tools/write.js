...
export function registerWriteTools(server, db, userId) {
    // -----------------------------------------------------------------------
    // create_task
    // -----------------------------------------------------------------------
    server.registerTool("create_task", {
        title: "Create Task",
        description: "Create a new task. Only title is required. Returns the created task's ID and title.",
        inputSchema: {
            title: z
                .string()
                .min(1)
    ...
    server.registerTool("update_task", {
        title: "Update Task",
        description: "Update one or more fields on an existing task. Only pass the fields you want to change.",
        inputSchema: {
        ...
        if (updates.dueDate !== undefined)
            setValues.dueDate = updates.dueDate;
        if (updates.estimatedMinutes !== undefined)
            setValues.estimatedMinutes = updates.estimatedMinutes;
        if (Object.keys(setValues).length === 0) {
            return {
                content: [
                    { type: "text", text: "No fields to update." },
                ],
                isError: true,
            };
        }
    ...
    server.registerTool("delete_task", {
        title: "Delete Task",
        description: "Soft-delete a task (sets deletedAt timestamp). The task is hidden but not permanently removed.",
        inputSchema: {
    ...
    server.registerTool("complete_task", {
        title: "Complete Task",
        description: "Toggle a task's completion status. If todo/in_progress -> done. If already done -> todo.",
        inputSchema: {
