...
import { tasks, TASK_STATUSES, PRIORITIES, TIME_BLOCKS, } from "../schema.js";
export function registerAiPoweredTools(server, db, userId) {
    // -----------------------------------------------------------------------
    // split_task
    // -----------------------------------------------------------------------
    server.registerTool("split_task", {
        title: "Split Task into Subtasks",
        description: "Break a parent task into multiple subtasks. Each subtask is created with parentId pointing to the original task. The parent task itself is not modified. Use this to decompose complex tasks into actionable steps.",
        inputSchema: {
        ...
        if (!parent) {
            return {
                content: [
                    { type: "text", text: `Parent task not found: ${taskId}` },
                ],
                isError: true,
            };
        }
        ...
        const created = await db
            ...
            .returning();
        const totalMinutes = created.reduce((sum, t) => sum + (t.estimatedMinutes ?? 0), 0);
    ...
    // arrange_today
    // -----------------------------------------------------------------------
    server.registerTool("arrange_today", {
        title: "Arrange Today's Schedule",
        description: "Batch-assign time blocks to multiple tasks at once. Use this to plan your day by distributing tasks across morning, afternoon, and evening blocks.",
        inputSchema: {
            assignments: z
                .array(z.object({
    ...
    server.registerTool("batch_update", {
        ...
        description: "Update multiple tasks at once. Each update specifies a taskId and the fields to change. Useful for bulk operations like setting priorities, changing statuses, or rescheduling.",
        inputSchema: {
            updates: z
                .array(z.object({
...
//# sourceMappingURL=ai-powered.js.map